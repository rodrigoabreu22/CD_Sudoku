...
def solve_sudoku(sudoku, original_sudoku):
    """Solve the given Sudoku puzzle."""
    while not sudoku.check(0):
        for row in range(9):
            for col in range(9):
                if original_sudoku.grid[row][col] == 0:
                    for num in range(1, 10):
                        if is_valid(sudoku, row, col, num):
                            sudoku.grid[row][col] = num
                            if solve_sudoku(sudoku, original_sudoku):
                                return sudoku
...
def is_valid(sudoku, row, col, num):
    ...
    if num in sudoku.grid[row]:
        return False

    if num in [sudoku.grid[r][col] for r in range(9)]:
        return False
...
if __name__ == "__main__":
    ...
    print(sudoku)
    sudoku_copy = Sudoku(sudoku.grid)
    sudoku = solve_sudoku(sudoku_copy, sudoku)
